...
class NotificacaoAdmin(models.Model):
    # TIPOS_NOTIFICACAO foi expandido para incluir tipos mais genéricos e um tipo para 'Motorista Incorreto'
    TIPOS_NOTIFICACAO = [
        ...
        ('info_geral', 'Informação Geral'), # Adicionado para notificações não específicas
        ('alerta_seguranca', 'Alerta de Segurança'), # Adicionado para alertas de segurança
    ]
    ...
    # Campos de contexto (todos opcionais e com max_length ajustado para flexibilidade)
    ...
    # Campo para dados brutos ou adicionais (JSONField é o ideal)
